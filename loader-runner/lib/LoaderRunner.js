...
function runSyncOrAsync(fn, context, args, callback) {
	...
	var innerCallback = context.callback = function() {
		...
		try {
			// 异步完成后才执行callback
			// 因此也需要异步loader完成后，调用this.async生成的this.callback，才能继续执行下一个loader
			// 但是等待异步完成过程中，webpack会执行其他module.build，因为compilation.buildQueue是一个异步队列，会"并发"处理多个module(当前发生[异步]，就会处理别的module)
